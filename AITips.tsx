...
export function AITips() {
  ...
  useEffect(() => {
    const fetchTips = async () => {
      try {
        const res = await fetch(`${BASE_URL}/ai-tips?t=${Date.now()}`, { headers: HEADERS, cache: 'no-store' });
        if (res.ok) {
      ...
      <main className="max-w-md mx-auto p-4 space-y-4">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {tipsData.tips.map((tip, index) => (
              <TipCard
                ...
                savings={`₹${tip.savings.toLocaleString('en-IN')}`}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-8 text-center border border-gray-200 dark:border-slate-700">
            <p className="text-gray-600 dark:text-gray-400">No tips available yet. Start tracking your electricity usage!</p>
          </div>
        )}
        ...
        {tipsData && (
          <motion.div
            ...
            className="bg-gradient-to-r from-blue-100 dark:from-blue-900/30 to-cyan-100 dark:to-cyan-900/30 border border-blue-300 dark:border-blue-700 rounded-xl p-4 text-center"
          >
